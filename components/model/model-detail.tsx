...
import { getDataset, type Model, type TreeNode } from "@/lib/registry-data"
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-muted/40 text-left text-xs text-muted-foreground">
            ...
            <th className="px-3 py-2 font-medium">크기 (Size)</th>
            <th className="px-3 py-2 font-medium">의미 (Information)</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
...
function stageIcon(stage: string): typeof Box {
  const s = stage.toLowerCase()
  if (s.includes("base")) return Box
  if (s.includes("fine")) return Layers
  if (s.includes("quant")) return Cpu
  if (s.includes("edge")) return Rocket
  return GitBranch
}
...
function Lineage({ nodes }: { nodes: TreeNode[] }) {
  return (
        ...
        return (
          <div key={node.id} className="relative flex gap-3 pb-5 last:pb-0">
            ...
            <div className="flex flex-1 flex-col gap-1 pt-0.5">
              <span className="font-medium leading-tight text-pretty">{node.name}</span>
            ...
            <CardContent className="flex flex-col p-0">
              ...
              <div className="grid grid-cols-2 border-b border-border md:grid-cols-3 lg:grid-cols-6">
                {[
                  ...
                  { icon: FileCode2, label: "모델 파일", value: model.modelFileExtension },
                  { icon: Server, label: "저장 위치", value: model.storagePath },
            ...
            <CardContent className="grid grid-cols-1 p-0 lg:grid-cols-2">
              ...
              <div className="flex min-h-[420px] flex-col gap-5 border-t border-border p-6 md:p-8 lg:border-l lg:border-t-0">
                ...
                <div className="flex flex-col divide-y divide-border/60">
                  ...
                  <SpecRow label="모델 파일" value={<span className="font-mono text-xs">{model.modelFileExtension}</span>} />
                  <SpecRow label="저장 위치" value={<span className="font-mono text-xs">{model.storagePath}</span>} />
                </div>
